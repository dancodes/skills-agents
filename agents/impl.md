...
- Typecheck only through `python3 "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/daniel/typecheck.py"`, run from your workspace directory with a Bash timeout of 600000. It takes the same arguments as `yarn typecheck`, waits for its turn, and then runs it. Every workspace shares this machine's RAM, tsgo peaks around 4GB, and several running at once thrash the machine until the OOM killer fires, so the script lets one through at a time. A hook blocks `yarn typecheck`, `tsgo` and `tsc` run directly.
