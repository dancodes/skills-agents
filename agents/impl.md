...
- Run the frontend integration suite as `VITEST_MAX_WORKERS=1 yarn test:integration`, and scope it to the paths you touched. The config sets `maxWorkers: 2`, which is right for one suite on this 2-core machine but pushes it to ~130% CPU; several workspaces running that at once starve each other. Scope it because a full `yarn test:integration` is over an hour.
